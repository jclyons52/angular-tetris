...
export class Block {
  ...
  add = () => {
    this.pixels.map(p => {
      const pixel = this.rows[p.y].pixels[p.x];
      pixel.filled = true;
    });
  }
  ...
  isFull = (row: Row): boolean => {
    return row.pixels.filter(i => !i.filled).length === 0;
  }
  ...
  isOverlapping = (): boolean => {
    return (
      this.pixels.filter(({ x, y }) => {
        ...
        if (x > this.rows[y].pixels.length) {
          return false;
        }
        return this.rows[y].pixels[x].filled;
      }).length > 0
    );
  }
  ...
  canMoveDown = (): boolean => {
    return (
      this.pixels.filter(({ x, y }) => {
        ...
        if (this.rows[y + 1].pixels[x].filled) {
          return true;
        }
        return false;
      }).length === 0
  ...
  canMoveLeft = (): boolean => {
    return (
      this.pixels.filter(({ x, y }) => {
        ...
        if (this.rows[y].pixels[x - 1].filled) {
          return true;
        }
        return false;
      }).length === 0
